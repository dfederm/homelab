...
# mfsymlinks: client-side symlink emulation for repository/config symlinks.
# CIFS otherwise rejects symlink() with EIO unless the server returns NFS
# reparse points, which Samba does not by default.
