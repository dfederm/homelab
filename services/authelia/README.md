...
**Open WebUI is the first protected surface**, wired via **OIDC** (see the recipe below). No
forward-auth surface is wired yet.
...
|---|---|---|
| `apollo.env` | `AUTHELIA_*` secrets + non-secret FQDN/port vars | See `.env.template` for the full list. |
...
### 1. Generate the scalar secrets → `apollo.env`
...
Assign the four values to, in `apollo.env`:

| `apollo.env` var | Purpose |
|---|---|
...
and groups. Put your admin account(s) in the `admins` group; leave low-sensitivity family accounts with
empty groups. Generate each password hash:
...
| Value | Where |
|---|---|
| Random Password (plaintext) | `apollo.env` → `OPEN_WEBUI_OIDC_CLIENT_SECRET` (Open WebUI reads this). Safe as an env var — the `rfc3986` charset contains no `$`. |
...
Add `authelia` to `HOMELAB_SERVICES` in `apollo.env` (and Open WebUI already carries its new OIDC
...
- Logging in as an `admins`-group user is challenged for **TOTP** (first login prompts enrollment);
  a family user logs in with password only.
...
3. **Choose an authorization policy.** Reuse `default_policy: one_factor` or add a per-app policy
   under `authorization_policies` (e.g. require `two_factor` for a `group:admins` subject) and set
   the client's `authorization_policy`.
